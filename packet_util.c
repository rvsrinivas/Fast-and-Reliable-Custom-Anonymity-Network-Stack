...
int generate_key_packet(u_char* packetOut, unsigned char* payload, int key_size, int size, int source, int dest) {
    ...
    int hdrlen;
    //printKey(payload, key_size);
    memset(packetOut, 0, sizeof(u_char) * PACKET_BUF_SIZE);
    struct ethhdr* eth = (struct ethhdr*) packetOut;
    struct rthdr* rth = (struct rthdr*)(packetOut + sizeof(struct ethhdr));
    switch (source) {
		case 1:
			eth->h_source[0] = 0x00; eth->h_source[1] = 0x15; eth->h_source[2] = 0x17;
			eth->h_source[3] = 0x57; eth->h_source[4] = 0xc7; eth->h_source[5] = 0x99;
			rth->saddr = (u_int16_t)(0x0011);
			break;
		case 2:
			eth->h_source[0] = 0x00; eth->h_source[1] = 0x15; eth->h_source[2] = 0x17;
			eth->h_source[3] = 0x57; eth->h_source[4] = 0xc3; eth->h_source[5] = 0xd2;
			rth->saddr = (u_int16_t)(0x0021);
			break;
		default:
			return -1;
	...
	switch (dest) {
		case 1:
			eth->h_dest[0] = 0x00; eth->h_dest[1] = 0x15; eth->h_dest[2] = 0x17;
			eth->h_dest[3] = 0x57; eth->h_dest[4] = 0xc7; eth->h_dest[5] = 0x99;
			rth->daddr = (u_int16_t)(0x0011);
			break;
		case 2:
			eth->h_dest[0] = 0x00; eth->h_dest[1] = 0x15; eth->h_dest[2] = 0x17;
			eth->h_dest[3] = 0x57; eth->h_dest[4] = 0xc3; eth->h_dest[5] = 0xd2;
			rth->daddr = (u_int16_t)(0x0021);
			break;
		default:
			return -1;
