...
void process_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    //printf("RECEIVED ENC PACKET \n");
    //print_rl_packet(stdout, packetIn, size);
	decrypt(packetIn, packet_d, size);
	//printf("DECRYPTED PACKET \n");
	//print_rl_packet(stdout, packet_d, size);

    struct rthdr *rth = (struct rthdr*) (packet_d + sizeof(struct ethhdr));
	char err[128];
	...
	switch(protocol) {
                    case ROUTE_ON_RELIABLE:
                        ...
                        //printf("hdrlen : %d \n", hdrlen);
                        struct rlhdr* rlh = (struct rlhdr*)(packet_d + sizeof(struct ethhdr) + sizeof(struct rthdr));
                        //printf("Port : %d \n", rlh->port);
                        if(rlh->port == 11){
                            if (updateTrackPacketsArray(track_packets1, rlh->seq)){
                            track_packets1[rlh->seq] = 1;
                            payload_size = size - hdrlen;
                            write_re_to_file1(packet_d + hdrlen, payload_size, rlh->seq);
                        }
                        }
                        else if(rlh->port == 12){
